...
class Murmur:
    ...
    def is_allowed_thread(self, thread_id: str) -> bool:
        return not self.settings.allowed_thread_ids or thread_id in self.settings.allowed_thread_ids
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if response.status >= 400:
                    raise RuntimeError(f"Open WebUI error {response.status}: {body}")
